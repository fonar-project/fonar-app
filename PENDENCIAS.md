...
| Gerar o laudo de novo substitui o anterior — desde 25/09/2026 a tela pergunta antes e avisa para salvar o PDF de antes, mas continua havendo UMA versão por análise. Laudo já entregue precisa ficar guardado como foi (versões)? Assinatura digital? | 10 | `laudo/domain/laudo.dart` |
...
| **Desligar o processamento automático é PEDIDO, não garantia.** O `RecordConfig` manda `autoGain: false`, `noiseSuppress: false` e `echoCancel: false`, e no Android a fonte `VOICE_RECOGNITION` — que a definição de compatibilidade obriga a chegar sem ganho automático nem redução de ruído. Nada disso é CONFIRMADO em tempo de execução: o `record` não devolve o que a plataforma aceitou, e o cabeçalho do WAV cobre formato (taxa, canais, bits), não processamento. Um fabricante que ignore o pedido altera exatamente as propriedades que a análise mede, e o app não tem como perceber. Só se confirma medindo em aparelho real — gravar o mesmo sinal conhecido em cada aparelho e comparar. | 04, 05 | `captura/data/configuracao_de_captura.dart` |
| **Trecho zerado no começo do arquivo.** A conferência trata como microfone mudo o trecho de áudio da janela de 4 KB já lida (~46 ms) todo em zero absoluto. Um microfone ligado nunca entrega zero exato — mas o primeiro buffer da captura pode. Conferir que gravação boa em aparelho real não cai nisso, e calibrar `bytesMinimosParaJulgarSilencio` se cair. | 05 | `captura/domain/verificacao_da_amostra.dart` |
| Nível do medidor e aferição no Android e no Windows, com o microfone do projeto. | 04 | idem |
| O WAV gravado sai PCM 16 bits, 44,1 kHz, mono — e a conferência aceita. A profundidade de bits é a única parte do formato que não se PEDE: vem embutida no encoder `wav`, e nem o `setOnConfigChanged` fala dela. A conferência compara o cabeçalho com `ConfiguracaoDeCaptura.bitsPorAmostra`, que é suposição sobre o encoder — o cabeçalho é a única evidência que existe. | 05 | `captura/data/gravador_record.dart`, `captura/data/configuracao_de_captura.dart` |
| Se o `setOnConfigChanged` avisa quando o aparelho troca a taxa. Desde 25/09/2026 ele é registrado nas DUAS capturas — a da aferição e a da gravação —, e na gravação o aviso já gera ressalva de formato por si, mesmo que o cabeçalho repita o que foi pedido. Nunca foi visto disparar. | 04, 05 | `captura/data/fonte_de_nivel_record.dart`, `captura/data/gravador_record.dart` |
